...
面向大学生的个人网页课表，手机端优先，兼容桌面浏览器。课表保存在浏览器本地，不需要注册登录。
...
新浏览器第一次打开部署后的地址时，会自动导入本项目开发浏览器中现有的课表：`2026-2027 学年第一学期`、12 节作息、7 门课程和 13 组重复安排。导入只执行一次：浏览器已有课表时优先保留已有数据，之后的修改、备份恢复或主动清空也不会被默认值覆盖。

课表仍然保存在每位访问者自己的浏览器 `localStorage` 中，Docker 服务端不保存用户修改。域名、协议或端口变化会被浏览器视为新的站点，因此会重新获得一份初始课表。当前项目没有登录鉴权，而且初始课表会进入公开的前端资源；部署到公网时请使用防火墙、反向代理认证或其他访问控制，避免无关人员访问。
...
- 备份格式版本升到 2，`formatVersion: 1` 的旧备份仍可恢复并自动升级；高于当前版本的备份被拒绝，且不会覆盖已有课表。
- 撤销记录属于会话状态，不进备份。
...
143 个用例，全部使用固定日期和受控响应，不依赖真实付费服务：
